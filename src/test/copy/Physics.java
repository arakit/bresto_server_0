			...
			for(Particle object : activeParticles) {
				if(subject != object){
					...
					Point2D.Double force = support.PointDivide(
								support.PointScale( direction, (repulsion * object.mass * 5) ) ,
									(distance * distance * 0.5 ) );

					subject.applyForce(force);

					force = support.PointDivide(
								support.PointScale( direction, (repulsion * subject.mass * 5) ) ,
									(distance * distance * -0.5 ) );
...
//			} else if (particle.position.x > w - (w / 8) || particle.position.x < -w + (w / 8) || particle.position.y > h - (h / 8) || particle.position.y < -h + (h / 8)) {
//				particle.applyForce(su.PointScale(direction, (repulsion / 80.0)));
//			} else if (particle.position.x > w - (w / 4) || particle.position.x < -w + (w / 4) || particle.position.y  > h - (h / 4) || particle.position.y  < -h + (h / 4)) {
//				particle.applyForce(su.PointScale(direction, repulsion / 90));
//			} else {
