...
public class DeleteKeyword extends HttpServlet {
	...
	private void doProc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			String _kid = CFUtil.getParam(request, "kid");
...
//	         }
			...
			if(kr==null) throw new Exception("not exist kid = "+kid);
			...
			HashSet<KeywordsRelationRow> del_rel = new HashSet<KeywordsRelationRow>();

			for(int i=0;i<krlist.size();i++){
				KeywordsRelationRow krr = krlist.get(i);
				if(krr.kid1==(int)kid || krr.kid2==(int)kid){
					del_rel.add(krr);
				}
			}

			for(KeywordsRelationRow krr : del_rel){
				db_kr.delete(krr);
			}


			boolean suc_del = db_k.delete(kid, project_id);

			if(!suc_del) throw new Exception("can not delete kid = "+kid);
