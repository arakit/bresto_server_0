...
public class CreateUser extends HttpServlet {
	...
	private void doProc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(user_id.length()<4) throw new Exception("length of id is 4 or more. ");
			if(user_password.length()<4) throw new Exception("length of password is 4 or more. ");
			if(user_id.length()>32) throw new Exception("length of id is 32 or less. ");
			if(user_password.length()>32) throw new Exception("length of password is 32 or less. ");
			...
			HttpSession ses = request.getSession();
	         ...
	         if(user_row!=null) throw new Exception("exist user.");

	         //if( !user_row.pass.equals(user_password) ) throw new Exception("correct password.");
	         
	         user_row = new UserRow();
	         user_row.id = user_id;
	         user_row.pass = user_password;
	         if( !tb_u.insert(user_row) )  throw new Exception("failed insert user.");
